...
def checkaolav(email):
    ...
    'validateField': 'userId',
        ...
        if '{"errors":[{"name":"firstName","error":"FIELD_EMPTY"},{"name":"lastName","error":"FIELD_EMPTY"},{"name":"birthDate","error":"INVALID_BIRTHDATE"},{"name":"password","error":"FIELD_EMPTY"}]}' in response:return {'by':'@cc_02','status':'good','email':email+'@aol.com'}
        elif 'IDENTIFIER_NOT_AVAILABLE' or 'LENGTH_TOO_SHORT' or 'CANNOT_START_WITH_SPECIAL_CHARACTER_OR_NUMBER' or 'CANNOT_END_WITH_SPECIAL_CHARACTER' or 'SOME_SPECIAL_CHARACTERS_NOT_ALLOWED' in response:return {'by':'@cc_02','status':'bad','email':email+'@aol.com'}
...
def chk(email):
	try:
		return jsonify(checkaolav(email))
